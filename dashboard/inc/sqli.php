...
class database
{
    ...
    public function wherelike($field, $equal = null)
    {
        if (is_array($field)) {
            self::__wherelike($field);
        } else {
            self::__wherelike(array($field => $equal));
        }
        return $this;
    }
    ...
    private function extra()
    {
        ...
        if (!empty($this->like)) $extra .= ' ' . $this->like;
